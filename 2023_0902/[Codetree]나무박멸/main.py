    ...
    for i in range(N):
        for j in range(N):
            if current_tree[i][j]:
                ...
                for di, dj in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
                    ni, nj = i + di, j + dj
                    if 0 <= ni < N and 0 <= nj < N and not current_tree[ni][nj] and not current_herbicide[ni][nj] and graph[ni][nj] >= 0:
                        cnt += 1
                        tmp.append((ni, nj))
    ...
    for di, dj in [(-1, -1), (-1, 1), (1, 1), (1, -1)]:
        ...
        while k <= K:
            ...
            if 0 <= ni < N and 0 <= nj < N:
                ...
                # 벽이나 나무가 0인 칸 만나면 더 이상 퍼지지 않음
                elif graph[ni][nj] == 0 or current_herbicide[ni][nj]:  
                    current_herbicide[ni][nj] = C
                    break
                else:
                    break
